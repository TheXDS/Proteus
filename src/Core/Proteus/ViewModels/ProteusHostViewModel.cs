...
/// </summary>
public abstract class ProteusHostViewModel : ViewModel
{
    ...
    /// this instance.
    /// </param>
    public ProteusHostViewModel(ITritonService service)
    {
        Service = service;
    ...
    protected CrudButtonInteraction CreateCrudInteraction<TDescriptor>(string label, string? group = null, bool essential = false, bool clearStack = false) where TDescriptor : ICrudDescriptor, new()
    {
        return new(() =>
        {
            if (clearStack) _childNavViewModel.NavigationService.Reset();
            _childNavViewModel.NavigationService.NavigateToCrud<TDescriptor>(Service);
        }, label)
