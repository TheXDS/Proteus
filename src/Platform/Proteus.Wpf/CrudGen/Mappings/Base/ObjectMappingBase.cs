    ...
    where TDescription : class, IObjectPropertyDescription
{
    ...
    {
        var m = models.Select(p => new NamedObject<ICrudDescription>(p.FriendlyName, p)).ToArray();
        var result = await dialogService!.SelectOption(CommonDialogTemplates.Input with { Title = St.NewItem, Text = St.NewItemHelp }, m);
